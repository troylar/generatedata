...
define([
  ...
  "lang",
  "moment"
], function (manager, pluginManager, utils, history, Queue, C, L, moment) {
  ...
  var _run = function () {
    ...
    $(".gdAddRowsBtn").bind("click", function () {
  ...
  var _updateVisibleRowNums = function () {
    $("#gdTableRows>li .gdColOrder").each(function (i) {
      $(this).html(i + 1);
    });
  };
  ...
  var _displayDataSets = function () {
    if (_dataSets.length) {
      ...
      for (var i = 0; i < _dataSets.length; i++) {
        ...
        '<td align="center"><a href="#" class="loadConfigurationHistory">history</a></td>' +
  ...
  var _initLoginDialog = function () {
    $("#gdLoginDialogTabs>ul>li").each(function () {
      ...
      $(this).bind("click", function () {
        utils.selectTab({
          tabGroup: "dialogTabs",
          tabIDPrefix: "gdLoginDialogTab",
          newTab: newTab,
          oldTab: _currLoginDialogTab
        });
  ...
  var _openLoginDialog = function () {
    ...
    $("#" + _loginModalID).dialog({
      ...
      modal: true,
      open: function () {
        utils.insertModalSpinner({modalID: _loginModalID});
      },
      buttons: buttons
    });
    $("#gdLoginError").hide();
  ...
  var _onClickLoadConfigurationHistory = function (e) {
    e.preventDefault();
    var configurationID = $(e.target).closest("tr").data("id");
    history.getHistory(configurationID);
  };
